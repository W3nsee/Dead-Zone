...
function syncClientZombies(serverZombies) {
    for (let i = zombiesArray.length - 1; i >= 0; i--) {
        let localZ = zombiesArray[i];
        if (!serverZombies.find(sz => sz.id === localZ.id)) {
            if(localZ.audio && localZ.audio.isPlaying) localZ.audio.stop();
            scene.remove(localZ.mesh);
    ...
    serverZombies.forEach(sz => {
        ...
        if (localZ) {
            ...
            let zObj = createZombieMesh();
            zObj.id = sz.id; zObj.maxHp = sz.maxHp; zObj.hp = sz.hp;
            zObj.mesh.position.set(sz.x, sz.y, sz.z);
            zObj.mesh.rotation.y = sz.rotY;
            
            let zSound = new THREE.PositionalAudio(audioListener);
            zSound.setRefDistance(5); zSound.setMaxDistance(50); zSound.setLoop(true); 
            zSound.setVolume(baseVolumes.zombie * globalVolMulti * 1.5); 
            if(zombieAudioBuffer) { zSound.setBuffer(zombieAudioBuffer); zSound.play(); }
            zObj.mesh.add(zSound); zObj.audio = zSound;
