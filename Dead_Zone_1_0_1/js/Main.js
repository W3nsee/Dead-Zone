...
let prevScreen = 'screen-main'; // Memoria para saber a dónde volver desde opciones
...
function showScreen(idToShow) {
    ...
    screens.forEach(s => { 
        ...
        if(el) {
            // Si vamos a Opciones, recordamos en qué pantalla estábamos
    ...
    // Ocultar el cartel de la sala si estamos navegando en los menús
...
// NUEVO: Función para preparar el código de sala en pantalla (Invisible hasta jugar)
function actualizarCartelSala(codigo, esHost) {
    ...
    if(!hud) {
        ...
        hud.style.display = 'none'; // Empieza oculto
...
addEvt('card-multiplayer', 'click', () => {
    ...
    if(!peer) {
        ...
        peer.on('open', (id) => { 
            ...
            actualizarCartelSala(id, true); // Prepara el cartel pero no lo muestra aún
        });
...
addEvt('btn-join-room', 'click', () => { 
    ...
    conn.on('open', () => { 
        ...
        actualizarCartelSala(roomCode.value.toUpperCase(), false); // Prepara el cartel
        iniciarPartidaMultijugador(); 
    }); 
});

// === CORRECCIÓN RATÓN REANUDAR ===
addEvt('btn-resume', 'click', () => { 
    if(!isMobile) controls.lock(); // 1. Atrapa el ratón INMEDIATAMENTE al hacer clic
    
    setTimeout(() => { // 2. Oculta la pantalla una fracción de segundo después para no bloquear el ratón
        let ui = document.getElementById('ui-layer'); if(ui) ui.style.display = 'none'; 
        resumeAudio(); isGameActive = true; prevTime = performance.now(); 
    }, 50);
});

// === CORRECCIÓN BOTÓN OPCIONES ===
addEvt('btn-options', 'click', () => { showScreen('screen-options'); }); 
addEvt('btn-options-pause', 'click', () => { showScreen('screen-options'); }); 
addEvt('btn-back', 'click', () => { showScreen(prevScreen); }); // Vuelve inteligentemente de donde viniste
...
function resetGame() {
    ...
    // Si estamos en multijugador y el juego acaba de empezar, mostramos el cartel
    let hud = document.getElementById('room-code-hud');
...
if(isMobile) {
    ...
    addEvt('btn-shoot', 'touchstart', (e)=>{ e.preventDefault(); if(isLobby) return; isShooting=true; let t = performance.now(); if(!armasConfig[armaActual].auto && t - lastShootTime >= armasConfig[armaActual].cooldownMs) { procesarDisparo(t); if(isMultiplayer && connections.length>0) connections[0].send({type:'shoot'}); } });
...
function animate() {
    ...
    if (isMultiplayer) {
        ...
        if (isHost) {
            ...
            if (Math.random() < 0.5 && connections.length > 0) { 
                connections.forEach(c => c.send({ type: 'world_state', hostId: myRoomId, players: otherPlayersData }));
            }
        } else {
            if (Math.random() < 0.5 && connections.length > 0) { 
    ...
    if (isLobby) {
        if (isHost && holdingStart) {
            ...
            if (holdStartProgress >= 1.5) { // 1.5 Segundos para empezar
        ...
        // IA ZOMBIES (Solo se ejecuta si la sala de espera terminó)
        ...
        for(let i = activeBullets.length - 1; i >= 0; i--) {
            let b = activeBullets[i]; b.life -= delta; if (b.life <= 0) { scene.remove(b.mesh); activeBullets.splice(i, 1); continue; } let moveDist = b.speed * delta; raycaster.set(b.mesh.position, b.dir); const intersects = raycaster.intersectObjects([...zombiesArray.map(z=>z.mesh), ...obstacles], true);
            if (intersects.length > 0 && intersects[0].distance <= moveDist) {
                let obj = intersects[0].object; if(obj.userData.isZombie) { let rootGroup = obj.parent; if(rootGroup.parent && rootGroup.parent.type === "Group") rootGroup = rootGroup.parent; let idx = zombiesArray.findIndex(z => z.mesh === rootGroup); if(idx !== -1) { let part = obj.userData.part || 'body'; let mult = (part === 'head') ? 2.0 : (part === 'limb') ? 0.7 : 1.0; damageZombie(idx, Math.round(b.dmg * mult), part, intersects[0].point); } }
        ...
        for(let i = activeGrenades.length - 1; i >= 0; i--) {
            ...
            g.timer--; if (g.timer <= 0) { let audio = audioBoom.cloneNode(); audio.volume = baseVolumes.boom * globalVolMulti; audio.play().catch(e=>{}); let fo = document.getElementById('flash-overlay'); if(fo) { fo.style.opacity = '0.8'; setTimeout(() => fo.style.opacity = '0', 100); } zombiesArray.forEach((z, idx) => { if(g.mesh.position.distanceTo(z.mesh.position) < 8) damageZombie(idx, 150, 'body', z.mesh.position); }); scene.remove(g.mesh); activeGrenades.splice(i, 1); }
        }
        ...
        zombiesArray.forEach(zObj => {
            let z = zObj.mesh; z.lookAt(camera.position.x, z.position.y, camera.position.z); zObj.hpGroup.lookAt(camera.position); const dirToPlayer = new THREE.Vector3().subVectors(camera.position, z.position); dirToPlayer.y = 0; const dist = dirToPlayer.length();
